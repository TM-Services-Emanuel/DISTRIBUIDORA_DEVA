...
public class Listar_IVA extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btnDOWNIVA).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AlertDialog.Builder alertDialog = new AlertDialog.Builder(Listar_IVA.this);
                alertDialog.setMessage("¿Seguro que desea descargar la lista completa de impuestos habilitados para esta aplicación?");
                alertDialog.setTitle("ACTUALIZACIÓN DE DATOS");
                alertDialog.setIcon(R.drawable.ic_descargarpcdark);
                alertDialog.setCancelable(false);
                alertDialog.setPositiveButton("SI, COMENZAR DESCARGA", (dialog, which) -> sincronizarIVA(Listar_IVA.this));
    ...
    private void AlertaEliminacion(){
        ...
        alertDialog.setCancelable(false);
        alertDialog.setPositiveButton("Sí", (dialog, which) -> eliminarIVA());
        alertDialog.setNegativeButton("Cancelar", (dialog, which) -> dialog.cancel());
        alertDialog.show();
    }
    ...
    public void llenarLista(){
        try{
            ...
            if(cant==0){
                ...
                pie.setText(String.valueOf(cant).concat(" impuesto listado".toUpperCase()));
            }else{
                pie.setText(String.valueOf(cant).concat(" impuestos listados".toUpperCase()));
            }
            db.close();
    ...
    private void sincronizarIVA(Context context) {
        if (checkNetworkConnection(context)) {
            StringRequest stringRequest = new StringRequest(Request.Method.GET, "https://"+IP+context.getResources().getString(R.string.URL_UPDATE_IVA), new Response.Listener<String>() {
                @Override
