...
public class Opciones_Mantenimiento extends AppCompatActivity {
    ...
    public void RespaldarVentas(final String jsonV, final String jsonD, Context context) {
        ...
        if (checkNetworkConnection(context)) {
            ...
            StringRequest stringRequest = new StringRequest(Request.Method.POST, "https://"+IP+getResources().getString(R.string.URL_SAVE_VENTAS), new Response.Listener<String>() {
                @Override
    ...
    public void RespaldarPE(final String json, Context context) {
        if (checkNetworkConnection(context)) {
            ...
            RequestQueue requestQueue = Volley.newRequestQueue(this);
            StringRequest stringRequest = new StringRequest(Request.Method.POST, "https://"+IP+getResources().getString(R.string.URL_SETUPDATE_PUNTOEMISION) + idPE, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    AlertDialog mDialog = new SpotsDialog.Builder()
                            .setContext(Opciones_Mantenimiento.this)
    ...
    public void ActualizarPuntoEmision(Context context) {
        if (checkNetworkConnection(context)) {
            ...
            StringRequest stringRequest = new StringRequest(Request.Method.GET, "https://"+IP+context.getResources().getString(R.string.URL_UPDATE_PUNTOEMISIONGENERAL)+idT, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Access_PE db = Access_PE.getInstance(context.getApplicationContext());
