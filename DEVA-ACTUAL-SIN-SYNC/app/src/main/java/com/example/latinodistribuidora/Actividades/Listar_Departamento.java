...
public class Listar_Departamento extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btnDOWNDEP).setOnClickListener(v -> {
            AlertDialog.Builder alertDialog = new AlertDialog.Builder(Listar_Departamento.this);
            alertDialog.setMessage("¿Seguro que desea descargar la lista completa de departamentos habilitados para esta aplicación?");
            alertDialog.setTitle("ACTUALIZACIÓN DE DATOS");
            alertDialog.setIcon(R.drawable.ic_descargarpcdark);
            alertDialog.setCancelable(false);
            alertDialog.setPositiveButton("SI, COMENZAR DESCARGA", (dialog, which) -> sincronizarDepartamentos(Listar_Departamento.this));
    ...
    public void llenarLista(){
        try{
            ...
            if(cant==0){
                ...
                pie.setText(cant+" departamento listado".toUpperCase());
            }else{
                pie.setText(cant+" departamentos listados".toUpperCase());
            }
            db.close();
    ...
    private void sincronizarDepartamentos(Context context) {
        if (checkNetworkConnection(context)) {
            StringRequest stringRequest = new StringRequest(Request.Method.GET, "https://"+IP+context.getResources().getString(R.string.URL_UPDATE_DEPARTAMENTOS), new Response.Listener<String>() {
                @Override
        ...
        try {
            ...
            if (cursor.moveToNext()) {
               ...
               // Toast.makeText(this, "IP DEL SERVIDOR NO ENCONTRADO.", Toast.LENGTH_SHORT).show();
                Snackbar.make(findViewById(R.id.linearLayout23), "IP DEL SERVIDOR NO ENCONTRADO.", Snackbar.LENGTH_SHORT)
                        .setAnimationMode(BaseTransientBottomBar.ANIMATION_MODE_SLIDE)
                        .setBackgroundTint(Color.parseColor("#11232E"))
                        .setActionTextColor(Color.parseColor("#FFFFFF")).show();
            }
            ...
            Snackbar.make(findViewById(R.id.linearLayout23), "TABLA DE SERVIDOR NO ENCONTRADO.", Snackbar.LENGTH_SHORT)
                    .setAnimationMode(BaseTransientBottomBar.ANIMATION_MODE_SLIDE)
                    .setBackgroundTint(Color.parseColor("#11232E"))
                    .setActionTextColor(Color.parseColor("#FFFFFF")).show();
        }
