...
public class Listar_ventas extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.id_imp_report).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                try{
                    sendDataReportComprobantes();
                    findViewById(R.id.id_imp_report).setVisibility(View.GONE);
                    findViewById(R.id.btn_id_cerrar_imp_comprobantes).setVisibility(View.VISIBLE);
                }catch(Exception e){}
            }
        });

        findViewById(R.id.btn_id_bt_comprobantes).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                try{
                    findBT();
                    openBT_2();
                    findViewById(R.id.btn_id_bt_comprobantes).setVisibility(View.GONE);
                    findViewById(R.id.id_imp_report).setVisibility(View.VISIBLE);
                }catch (Exception e){}
            }
        });

        findViewById(R.id.btn_id_cerrar_imp_comprobantes).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                try {
                    closeBT();
                    findViewById(R.id.id_imp_report).setVisibility(View.GONE);
                    findViewById(R.id.btn_id_bt_comprobantes).setVisibility(View.GONE);
                    findViewById(R.id.btn_id_bt_comprobantes).setVisibility(View.VISIBLE);
                    finish();
                }catch (Exception e){
    ...
    public void openBT_2() throws IOException {
        try {
            UUID uuid = UUID.fromString("00001101-0000-1000-8000-00805f9b34fb");
            mmSocket = mmDevice.createRfcommSocketToServiceRecord(uuid);
            mmSocket.connect();
        ...
        try {
            if (mmSocket != null) {
                try {
                    ...
                    String msg1 = "VENTA DE LACTEOS Y EMBUTIDOS\n";
                    ...
                    for(int i=1; i<=listaparaReimpresion.size();i++) {
                        ...
                        msg2 += String.format("%1$-9s %2$-12s %3$-14s %4$-10s" , item.getImpuesto()+"%",item.getCantidad()+" "+item.getUm(), nformat.format(item.getPrecio()), nformat.format(item.getTotal()));
                    }
                    msg2 +="\n";
    ...
    public void sendDataReportComprobantes(){
        try {
            if (mmSocket != null) {
                try {
                    //String msg = "\n";
                    ...
                    for(int i=1; i<=listaR.size();i++) {
                        int position = (i - 1);
                        Ventas item = listaR.get(position);
                        String est;
                        if(item.estado.equals("N")){
                            est="(ANULADO)";
                        }else{
                            est="";
                        }
                        msg2+="- FACT. "+item.condicion+" NRO: "+item.Est+"-"+item.Emision+"-"+item.nrofactura+" "+est+"\n  MONTO FACTUADO: "+nformat.format(item.total)+"\n";
                        }
                    msg2+= "------------------------------------------------\n";
                   // findViewById(R.id.id_imp_report).setVisibility(View.GONE);
                    //findViewById(R.id.btn_id_cerrar_imp_comprobantes).setVisibility(View.VISIBLE);
                    ...
                    String msg4 ="\n";
                    msg4 += "------------------- RESUMEN --------------------"+"\n";
                    msg4 += "CANTIDAD: "+((TextView)findViewById(R.id.id_ventas_pie)).getText()+"\n";
                    msg4 += "MONTO "+((TextView)findViewById(R.id.txtTotalVR)).getText()+"\n";
                    msg4 += "MONTO "+((TextView)findViewById(R.id.txtTotalVR2)).getText()+"\n\n";
                    mmOutputStream.write(ESC_ALIGN_LEFT);
                    mmOutputStream.write(0x1C); mmOutputStream.write(0x2E); // Cancelamos el modo de caracteres chino (FS .)
                    mmOutputStream.write(0x1B); mmOutputStream.write(0x74); mmOutputStream.write(0x10); // Seleccionamos los caracteres escape (ESC t n) - n = 16(0x10) para WPC1252
                    mmOutputStream.write(getByteString(msg4,0,0,0,0));
