...
public class Listar_Division extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btnDOWNCLAS).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AlertDialog.Builder alertDialog = new AlertDialog.Builder(Listar_Division.this);
                alertDialog.setMessage("¿Seguro que desea descargar la lista completa de clasificaciones habilitadas para esta aplicación?");
                alertDialog.setTitle("ACTUALIZACIÓN DE DATOS");
                alertDialog.setIcon(R.drawable.ic_descargarpcdark);
                alertDialog.setCancelable(false);
                alertDialog.setPositiveButton("SI, COMENZAR DESCARGA", (dialog, which) -> sincronizarDivision(Listar_Division.this));
    ...
    private void AlertaEliminacion(){
        ...
        alertDialog.setCancelable(false);
        alertDialog.setPositiveButton("Sí", (dialog, which) -> eliminarDivision());
        alertDialog.setNegativeButton("Cancelar", (dialog, which) -> dialog.cancel());
        alertDialog.show();
    }
    ...
    public void llenarLista(){
        try{
            ...
            if(cant==0){
                ...
                pie.setText(String.valueOf(cant).concat(" clasificación listada".toUpperCase()));
            }else{
                pie.setText(String.valueOf(cant).concat(" clasificaciones listadas".toUpperCase()));
            }
            db.close();
    ...
    private void sincronizarDivision(Context context) {
        if (checkNetworkConnection(context)) {
            StringRequest stringRequest = new StringRequest(Request.Method.GET, "https://"+IP+context.getResources().getString(R.string.URL_UPDATE_DIVISION), new Response.Listener<String>() {
                @Override
    ...
    public void ObtnerIP() {
        try {
            ...
            Snackbar.make(findViewById(R.id.linearLayout31), "TABLA DE SERVIDOR NO ENCONTRADO.", Snackbar.LENGTH_SHORT)
                    .setAnimationMode(BaseTransientBottomBar.ANIMATION_MODE_SLIDE)
                    .setBackgroundTint(Color.parseColor("#11232E"))
                    .setActionTextColor(Color.parseColor("#FFFFFF")).show();
        }
